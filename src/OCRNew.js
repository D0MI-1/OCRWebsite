        ...
        for (const file of files) {
            try {
                ...
                if (fullTextAnnotation) {
                    ...
                    const { keywordResults, allWords } = processKeywords(fullTextAnnotation, keywords, companyName);
                    results.push({
                        fileName: file.name,
                        companyName,
    ...
    return (
        <div style={{ padding: '20px' }}>
            ...
            <div {...getRootProps()} style={{ border: '2px dashed #cccccc', padding: '20px', textAlign: 'center', cursor: 'pointer', marginBottom: '20px' }}>
                <input {...getInputProps()} accept=".jpg,.jpeg,.png,.pdf" multiple />
            ...
            <button onClick={handleFileUpload} disabled={files.length === 0 || isLoading} style={{ marginRight: '10px' }}>
                {isLoading ? "Processing..." : "Process All Files"}
            </button>
            <button onClick={clearFiles} disabled={files.length === 0 || isLoading}>
                Clear Files
            </button>
                            ...
                            {result.error ? (
                                ...
                                <>
                                    ...
                                    {result.keywordResults && (
                                        <ul>
                                            {Object.entries(result.keywordResults).map(([keyword, value]) => (
                                                <li key={keyword}>
                                                    <strong>{keyword}:</strong> {value.found ? `${value.value} (Position: x=${value.position.x.toFixed(2)}, y=${value.position.y.toFixed(2)})` : 'Not found'}
                                                </li>
                                            ))}
                                        </ul>
                                    )}
                                    ...
                                    <details>
                                        <summary>Show all recognized words with positions</summary>
                                        <ul style={{ maxHeight: '300px', overflowY: 'auto', border: '1px solid #ccc', padding: '10px' }}>
                                            {result.allWords.map((word, wordIndex) => (
                                                <li key={wordIndex}>
